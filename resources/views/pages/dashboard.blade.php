@extends('layouts.url')

@section('title', 'Dashboard')

@section('content')

<?php 
    $date = date('d/m/Y');
    $newDate = date("d/m/Y",strtotime($date."-3 month")); 
?>
        
    
    <div id="dashboard-content">

        <div class="header">
            <h2 class="text-color1 mt-5 rounded bg-white p-4 som">Panel de control landings</h2>
        </div>

        <div class="d-flex justify-content-end mt-5 flex-wrap">

            @if($role == '1')
            <div class="pSelect mt-5 d-flex justify-content-center align-items-center mr-5">
                <label for="byLanding" class="mr-3 mt-2 text-color1">LANDING</label>
                <select id="byLanding" class="som select mr-5" multiple>
                    @foreach ($landings as $l )
                        <option value="{{$l->id}}">{{$l->name}}</option>
                    @endforeach
                </select> 

            </div>
            @endif
            
            <div class="pSelect mt-5 d-flex justify-content-center align-items-center mr-5">
                <label for="byRut" class="mr-3 mt-2 text-color1">RUT</label>
                <select id="byRut" class="som select mr-5" multiple>
                    @foreach ($ruts as $r )
                        <option>{{$r}}</option>
                    @endforeach
                </select> 

            </div>

            <div class="pSelect mt-5 d-flex justify-content-center align-items-center mr-5">
                <label for="byPhone" class="mr-3 mt-2 text-color1">TELÉFONO</label>
                <select  id="byPhone" class="som select mr-5" multiple>
                    @foreach ($phones as $p )
                        <option>{{$p}}</option>
                    @endforeach
                </select> 

            </div>
                         

            <input id="startDate" type="text" class="mt-5 datepicker som mr-5" value="<?php echo $newDate; ?>"/>
            <input id="endDate" type="text" class="mt-5 datepicker som mr-5" value="<?php echo $date; ?>"/>            
        </div>
        
        <div class=" p-5 flex-wrap">
            <div class="d-flex justify-content-between col-12">
                <div class="pcard som rounded p-5 bg-white ml-5">
                    <h3 class="text-right ">Total de landings</h3>
                    <div class="d-flex justify-content-between">
                        <img class="mt-3" width="50" height="50" src="data:image/png;base64," alt="">
                        <p id="totalLandings" class="mt-2"></p>
                    </div>
                </div>
    
                <div class="pcard som rounded p-5 bg-white ml-5">
                    <h3 class="text-right ">Total de visitas</h3>
                    <div class="d-flex justify-content-between">
                        <img class="mt-3" width="50" height="50" src="data:image/png;base64," alt="">
                        <p  id="totalVisitas" class="mt-2"></p>
                    </div>
                </div>
    
                <div class="pcard som rounded p-5 bg-white ml-5">
                    <h3 class="text-right ">Total de formularios</h3>
                    <div class="d-flex justify-content-between">
                        <img class="mt-3" width="50" height="50" src="data:image/png;base64," alt="">
                        <p  id="totalFormularios" class="mt-2"></p>
                    </div>
                </div>
    
                <div class="pcard som rounded p-5 bg-white ml-5">
                    <h3 class="text-right ">Total de Eventos</h3>
                    <div class="d-flex justify-content-between">
                        <img class="mt-3" width="50" height="50" src="https://img.icons8.com/nolan/64/000000/event-accepted.png" alt="">
                        <p  id="totalEventos" class="mt-2"></p>
                    </div>
                </div>

            </div>
            <div class="d-flex justify-content-between col-12">

                <div id="chart1" class="w-50 pcard mt-5 p-0 som"></div>
                <div id="chart2" class="w-50 pcard ml-5 mt-5 p-0 som"></div>

            </div>

            <div class=" col-12 mt-5 p-0">

                <h1 class="bg-color1 p-4 text-white">Landings Pages</h1>

                <table id="tablaLandings" class="p-0" data-toggle="table" data-height="500" data-pagination=true data-search=true  data-toolbar="#toolbar" data-show-export="true" data-locale='ES'>
                    <thead>
                        <tr>
                            <th data-field="id">ID</th>
                            <th data-field="name">Nombre</th>
                            <th data-field="client">Cliente</th>
                            <th data-field="url">Url</th>
                        </tr>
                    </thead>
                    <tbody>
                        
                    </tbody>
                    
                </table>

                <h1 class="bg-color6 p-4 text-white" class="mt-4">Vistas</h1>

                <table id="tablaVisitas" class="p-0" data-toggle="table" data-height="500" data-pagination=true data-search=true  data-toolbar="#toolbar" data-show-export="true">
                    <thead>
                        <tr>
                            <th data-field="id">ID</th>
                            <th data-field="name">Landing</th>
                            <th data-field="fecha">Fecha</th>
                            <th data-field="datos">Datos</th>
                        </tr>
                    </thead>
                    <tbody>
                        
                    </tbody>
                    
                </table>

                 <h1 class="bg-color4 p-4 text-white" class="mt-4">Formularios</h1>

                <table id="tablaFormularios" class="p-0" data-toggle="table" data-height="500" data-pagination=true data-search=true  data-toolbar="#toolbar" data-show-export="true">
                    <thead>
                        <tr>
                            <th data-field="id">ID</th>
                            <th data-field="name">Landing</th>
                            <th data-field="fecha">Fecha</th>
                            <th data-field="datos">Datos</th>
                        </tr>
                    </thead>
                    <tbody>
                        
                    </tbody>
                    
                </table>

                 <h1 class="bg-color2 p-4 text-white" class="mt-4">Eventos</h1>

                <table id="tablaEventos" class="p-0" data-toggle="table" data-height="500" data-pagination=true data-search=true  data-toolbar="#toolbar" data-show-export="true">
                    <thead>
                        <tr>
                            <th data-field="id">ID</th>
                            <th data-field="event">Evento</th>
                            <th data-field="name">Landing</th>
                            <th data-field="fecha">Fecha</th>
                            <th data-field="datos">Datos</th>
                        </tr>
                    </thead>
                    <tbody>
                        
                    </tbody>
                    
                </table>

            </div>


        </div>

    </div>

  
  
@endsection